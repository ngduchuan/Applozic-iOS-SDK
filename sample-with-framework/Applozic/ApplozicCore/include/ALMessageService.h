...
+ (void)getLatestMessageForUser:(NSString *)deviceKeyString withCompletion:(void(^)(NSMutableArray *messages, NSError *error)) completion;
...
+ (void)getMessageSENT:(ALMessage *)message withCompletion:(void (^)(NSMutableArray *messages, NSError *error))completion;
...
/// Used for sync the messages where meta data is updated.
...
/// Used for updating message metadata.
...
/// Used for fetching messages based on the message keys.
